...
class Response(object):
    def __init__(self, content, reply=False, embed=None, delete_after=0, delete_invoking=False):
...
class RH1(discord.Client):
    def __init__(self):
        ...
        self.has_transitioned_mmdb = True
        ...
        self.dm_msg = "Hello and welcome to the Rhino Bot Help Discord!\nPlease take a moment to review the rules in <#{rules}> and don't forget to assign yourself a role in <#{roleswap}> as you cannot send messages until you do, if you have any further questions, simply message this bot back to send a mod mail to the server staff!".format(rules=CHANS['rules'], roleswap=CHANS['roleswap'])
        ...
        self.scheduler.add_job(self.reorder_lfg_channels, 'cron', id='reorder_lfg',  second='*/2')
    ...
    async def queue_timed_ban_role(self, sec_time, user, timed_role, role_id, user_id):
        ...
        try:
            ...
            if timed_role in user.roles:
                if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: await user.remove_roles(timed_role, atomic = True)
        except:
            traceback.print_exc()
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(str(resp["stream"]["preview"]["large"])) as r:
                ...
                with open("img.png", "wb") as f:
                    f.write(data)
        file = discord.File("/home/bots/RH1/img.png", filename="img.png")
    ...
    async def reorder_lfg_channels(self):
        for cat_id in VC_CATS.values():
            ...
            vcs = [[float(channel.name.split()[-1] if "🕒" not in channel.name else channel.name.split()[-2]) if len(channel.members) > 0 else float(int(channel.name.split()[-1] if "🕒" not in channel.name else channel.name.split()[-2])/100), channel] for channel in cat.channels if isinstance(channel, discord.VoiceChannel)]
        ...
        while True:
            ...
            while ticker != 1800:
                ...
                if not [member_id for member_id in self.mod_mail_db if not self.mod_mail_db[member_id]['answered']]:
                    ...
                    ticker+=1
            try:
                async for lmsg in self.get_channel(CHANS['notifications']).history(limit=1):
                        if self.last_modmail_msg and lmsg.id == self.last_modmail_msg.id:
                            ...
                            self.last_modmail_msg = await self.safe_send_message(self.get_channel(CHANS['notifications']), content='There are **{}** unread items in the mod mail queue that\'re over a half hour old! Either run `!mmqueue` to see them and reply or mark them read using `!markread`!'.format(len([member_id for member_id in self.mod_mail_db if not self.mod_mail_db[member_id]['answered']])))
            except:
    ...
    async def check_twitch_streams(self):
        ...
        while True:
            for streamer in self.twitch_watch_list:
                try:
                    async with aiohttp.ClientSession(headers={"Client-ID": self.twitch_client_id}) as session:
                        ...
                        if resp and "stream" in resp and resp["stream"]:
                            ...
                            if streamer not in self.twitch_is_live:
                                ...
                                self.twitch_is_live[streamer] = {'detected_start': datetime.utcnow(),
                                                                 ...
                                                                 'message': None}
                                self.twitch_is_live[streamer]['message'] = await self.safe_send_message(self.get_channel(target_channel), embed=self.twitch_is_live[streamer]['embedded_object'], file=discord.File("/home/bots/RH1/img.png", filename="img.png"))
                            else:
    ...
    def load_modmail_logs(self):
        if not self.has_transitioned_mmdb:
            return {int(key): value for key, value in load_json('modmaildbbackup.json').items()}
        modmail_logs = {}
    ...
    async def get_or_create_modmail_channel(self, user_id, creation_state='new'):
        ...
        for category_id in CATS.values():
        ...
        category = discord.utils.get(self.get_all_channels(), id=CATS[f"mm{creation_state}"])
    ...
    async def transition_modmail_channels(self):
        ...
        modmail_category_cleared = {category_id: False for category_id in CATS.values()}
        ...
        try:
            while any(value == False for value in modmail_category_cleared.values()):
                ...
                for category_id in list(reversed(CAT_TRANSITION_ORDER)):
                    category = discord.utils.get(self.get_all_channels(), id=category_id)
                    ...
                    if len(channels) > 45:
                        ...
                        if CAT_TRANSITION[category_id] != "delete":
                            transition_category = discord.utils.get(self.get_all_channels(), id=CAT_TRANSITION[category_id])
    ...
    async def voice_channel_resizing(self):
        ...
        while True:
            ...
            for category_id in self.vc_tracker:
                ...
                baseline_channel_bucket = len(self.vc_tracker[category_id]) if len(self.vc_tracker[category_id]) < baseline_bucket_count else baseline_bucket_count
                #if self.lfg_vc_debug: print(f"RESIZER: Cat {CATS_REV[category_id]} baseline chan bucket - {baseline_channel_bucket}")
                if baseline_channel_bucket < baseline_bucket_count:
                    for _ in range(baseline_bucket_count-baseline_channel_bucket):
                ...
                empty_channel_bucket = empty_channel_bucket if empty_channel_bucket < empty_bucket_count else empty_bucket_count
                #if self.lfg_vc_debug: print(f"RESIZER: Cat {CATS_REV[category_id]} empty chan bucket - {empty_channel_bucket}")
                if empty_channel_bucket < empty_bucket_count:
                    for _ in range(empty_bucket_count-empty_channel_bucket): 
                        await self.queue_channel_for_creation(category_id)
                ...
                for channel_id in self.vc_tracker[category_id]:
                    ...
                    if channel_obj["mem_len"] < 1 and empty_channel_bucket != 0:
                        ...
                        if channel_obj["pending_future"]:
                            ...
                            if "🕒" in chan.name:
                                await self.safe_edit_channel(chan, name=chan.name[:-2])
    ...
    async def queue_channel_for_creation(self, category_id):
        ...
        await chan.edit(user_limit=5, bitrate=128000)
    ...
    async def queue_channel_for_deletion(self, category_id, channel_id, last_event):
        ...
        chan = self.get_channel(channel_id)
        await self.safe_edit_channel(chan, name=f"{chan.name} 🕒")
    ...
    async def on_ready(self):
        ...
        self.vc_categories = {'vc_cat': self.get_channel(VC_CATS['vc_cat'])}
        ...
        for user_id, timestamp in temp_dict.items():
            ...
            if user:
                try:
                    if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: 
            ...
            for user_id, timestamp in user_blob.items():
                ...
                if user:
                    try:
                        ...
                        print('cannot add role to %s, permission error' % user.name)
                    # try:
                        # if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = list())
                    # except discord.Forbidden:
                        # print('cannot add role to %s, permission error' % user.name)
        # print('Done!\n\nCacheing Avatars...')
        ...
        await self.safe_send_message(self.get_channel(CHANS['notifications']), content='I have just been rebooted!')
    ...
    def mods_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if [role for role in orig_msg.author.roles if role.id  in [ROLES['help']]] or (is_tag and (self.tags[is_tag][0] == 'unrestricted_eval' or [role for role in orig_msg.author.roles if role.id  in [ROLES['help']]])):
    ...
    async def cmd_shrink(self, message, channel, author):
        ...
        """
        if author.voice and author.voice.channel and author.voice.channel.name.lower().startswith(("[na", "[tts", "[eu", "[latam", "[apac")):
    ...
    async def cmd_clear(self, message, author, channel):
        ...
        if mod_check:
            ...
            author_roles = []
        if not ROLES['help'] in [role.id for role in author.roles]: await author.edit(roles = author_roles)
    ...
    @mods_only
    async def cmd_updateusers(self, guild):
        """
        """
        role = discord.utils.get(guild.roles, id=602708843536842764)
        for mbr in guild.members:
            if role not in mbr.roles:
                await mbr.add_roles(role, reason="adding check role to everyone on the server right now")
                print(mbr.name)
        return Response(':thumbsup:', reply=True)
    ...
    async def cmd_tag(self, message, guild, author, channel, mentions, leftover_args, eval_content):
        ...
        if switch in ['+', 'add', '-', 'remove', 'list', 'blacklist']:
            if switch in ['+', 'add']:
                if [role for role in author.roles if role.id  in [ROLES['help']]]:
                ...
                if tag_name.lower() == tag.lower():
                    if self.tags[tag][0]:
                        ...
                        if "unrestricted_eval" in tag_flags:
                            ...
                            return
                        elif "restrict" in tag_flags and not [role for role in author.roles if role.id  in [ROLES['help']]]:
                            return
    ...
    async def cmd_staff(self, message, channel, author, guild):
        ...
        """
        check_msg = await self.safe_send_message(channel, content=f'Please remember to *only* tag staff for things that **absolutely** require **immediate** attention and can only be addressed by server administration.\n\nFor non-immediate topics, *please* send a private message to {guild.me.mention} to send mod-mail to the administration.\n\nPlease react if you still wish to ping the staff')
        ...
        if str(reac.emoji) == '✅':
            role = discord.utils.get(guild.roles, id=ROLES['bots'])
            await role.edit(mentionable=True)
            mention_msg = await self.safe_send_message(channel, content='<@&{}>'.format(ROLES['bots']))
            await asyncio.sleep(1)
    ...
    async def cmd_cmdinfo(self, author, command=None):
        ...
        if command:
            ...
            if cmd:
                ...
                return Response(
                    "```{}\n{}```".format("\nThis command can only be ran by mods" if hasattr(cmd, 'mod_cmd') else "",
    ...
    async def cmd_markread(self, channel, author, guild,  auth_id=None):
        ...
        if auth_id in self.mod_mail_db:
            ...
            target_category = discord.utils.get(self.get_all_channels(), id=CATS[f"mmread"])
    ...
    async def cmd_r(self, channel, author, guild, raw_leftover_args):
        ...
        if member:
            ...
            target_category = discord.utils.get(self.get_all_channels(), id=CATS[f"mmwfr"])
    ...
    async def cmd_unmute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                ...
                if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = [role for role in user.roles if role.id not in UNPROTECTED_ROLES])
                try:
                    await user.edit(mute=False)
                except:
                    pass
    ...
    async def cmd_cban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                user = guild.get_member(user.id)
                if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = list(set(([role for role in user.roles] + [role]))))
            except discord.Forbidden:
    ...
    async def cmd_mute(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                user = guild.get_member(user.id)
                if not ROLES['help'] in [role.id for role in user.roles] and not user.bot: await user.edit(roles = list(set(([role for role in user.roles if role.id not in UNPROTECTED_ROLES] + [mutedrole]))))
    ...
    async def cmd_userinfo(self, guild, channel, message, author, mentions, raw_leftover_args):
        ...
        if all([True if role.id not in [ROLES['help']] else False for role in author.roles ]) and member.id != author.id:
            raise CommandError("Non-mods cannot request userinfo for users besides themselves")
        try:
        ...
        if member:
            ...
            if member_profile.hypesquad: 
                em.add_field(name='User In HypeSquad', value=f'{str(self.get_emoji(REACTS["hypesquad"]))}', inline=True)
            if member_profile.partner: 
                em.add_field(name='User Is a Partner', value=f'{str(self.get_emoji(REACTS["partner"]))}', inline=True)
            if member_profile.staff: 
                em.add_field(name='User Is Staff', value=f'{str(self.get_emoji(REACTS["staff"]))}', inline=True)
        ...
        try:
            ...
            async for entry in self.get_channel(CHANS['roleswap']).history(limit=10000):
                if entry.author == self.user:
                    await self.safe_delete_message(entry)
                    await asyncio.sleep(0.21)
        em = discord.Embed(description=f'React with each of the bots you need help with by clicking on the corresponding platform icon below.\n\n{str(discord.utils.get(self.emojis, id=REACTS["music"]))} for the Music Bot\n{str(discord.utils.get(self.emojis, id=REACTS["rsb"]))} for Rhino Server Bot', colour=discord.Colour(0x9d9bf4))
        em.set_author(name="Role Assignment")
        msg = await self.safe_send_message(self.get_channel(CHANS['roleswap']), embed=em)
        await msg.add_reaction(self.get_emoji(REACTS['music']))
        await msg.add_reaction(self.get_emoji(REACTS['rsb']))
        
        await self.safe_send_message(self.get_channel(CHANS['roleswap']), content='__                                                                                                          __\n:clock3: You will need to be registered on Discord for 5 minutes before selecting a role.\n:warning: Please refresh Discord using Ctrl+R if the reactions are not visible.')
    ...
    async def cmd_ban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            try:
                if  ROLES['help'] in [role.id for role in discord.utils.get(guild.members, id=user.id).roles]: return Response('Error: User is Staff!')
    ...
    async def cmd_softban(self, message, guild, author, mentions, raw_leftover_args):
        ...
        for user in users:
            ...
            # except:
                # pass
            try:
                if  ROLES['help'] in [role.id for role in user.roles]: return Response('Error: User is Staff!')
    ...
    async def log_action(self, user, action, *, message=None, after = None):
        ...
        # elif action == 'avatar_change':
            ...
            # if ffmpeg_success:
                ...
                # file = discord.File("/home/bots/RH1/output.gif", filename="output.gif")
    ...
    async def on_raw_reaction_remove(self, payload):
        ...
        if channel_id == CHANS['roleswap']:
            ...
            if member and [role for role in member.roles if role.id == ROLES['muted']]:
                return
            if emoji.id == REACTS['rsb']:
                await self.http.remove_role(SERVERS['main'], user_id, ROLES['rsb'], reason=None)
            if emoji.id == REACTS['music']:
                await self.http.remove_role(SERVERS['main'], user_id, ROLES['music'], reason=None)
            
    async def on_guild_channel_delete(self, channel):
        if channel.id in self.cached_modmail_channels.values():
            new_cached_values = {key:val for key, val in self.cached_modmail_channels.items() if val != channel.id}
            self.cached_modmail_channels = new_cached_values
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if channel_id == CHANS['roleswap']:
            ...
            if member and [role for role in member.roles if role.id == ROLES['muted']]:
                return
            if emoji.id == REACTS['rsb']:
                await self.http.add_role(SERVERS['main'], user_id, ROLES['rsb'], reason=None)
            if emoji.id == REACTS['music']:
                await self.http.add_role(SERVERS['main'], user_id, ROLES['music'], reason=None)
    ...
    async def on_reaction_add(self, reaction, member):
        ...
        if reaction.message.channel.id in [CHANS['drama']]:
            ...
            if ROLES['help'] in [role.id for role in user.roles]: 
            ...
            if reaction.emoji.id == REACTS['24mute']:
                ...
                await self.safe_send_message(self.get_channel(CHANS['modmail']), content=MSGS['modmailaction'].format(action_log_id=CHANS['actions'], username=user.mention, id=user.id, reason='as they were muted for 24 hrs'))
            ...
            if reaction.emoji.id == REACTS['48mute']:
                ...
                await self.safe_send_message(self.get_channel(CHANS['modmail']), content=MSGS['modmailaction'].format(action_log_id=CHANS['actions'], username=user.mention, id=user.id, reason='as they were muted for 48 hrs'))
                asyncio.ensure_future(self.queue_timed_mute(172800, user, mutedrole, user.id))
            if reaction.emoji.id == REACTS['ban']:
                try:
                    if  ROLES['help'] in [role.id for role in user.roles]: return
                    em = discord.Embed(colour=discord.Colour(0xFF0000), description=MSGS['banmsg'])
                    await self.safe_send_message(user, embed=em)
                except:
                    if not user:
    ...
    async def on_member_join(self, member):
        ...
        if updated_member_roles:
            if not ROLES['help'] in [role.id for role in member.roles]:
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before != after:
            if not (before and before.channel) and after and after.mute and not ROLES['help'] in [role.id for role in member.roles]:
                try:
            ...
            optional_content = ''
            staff_role = False
            role = discord.utils.get(member.guild.roles, id=ROLES['help'])
            ...
            if self.voice_changes[member.id]['changes'] < 10:
                ...
                try:
                    ...
                    pass
                if not ROLES['help'] in [role.id for role in member.roles]: await member.edit(roles = member_roles) 
            else:
    ...
    async def on_message(self, message, edit=False):
        ...
        if isinstance(message.channel, discord.abc.PrivateChannel):
            ...
            if message.author.id in self.mod_mail_db:
                ...
                target_category = discord.utils.get(self.get_all_channels(), id=CATS[f"mmreply"])
                mm_symbol = "reply"
                ...
                target_category = discord.utils.get(self.get_all_channels(), id=CATS[f"mmnew"])
                mm_symbol = "new"
        ...
        try:
            this = [role for role in message.author.roles if role.id  in [ROLES['help'], ROLES['bots']]]
        except:
            message.author.roles = [role for role in message.author.roles if role is not None]
            
        if not message.author.bot and not [role for role in message.author.roles if role.id  in [ROLES['help'], ROLES['bots']]]:
                ...
                if not drama_matches and not dox_matches:
                    ...
                    for msg_id, msg_dict in self.watched_messages.items():
                        if msg_dict['message_id'] == message.id:
                            await self.safe_delete_message(await self.get_channel(CHANS['drama']).fetch_message(msg_id))
            if drama_matches:
        ...
        if not message.author.bot and not [role for role in message.author.roles if role.id  in [ROLES['devs'], ROLES['help'], ROLES['bots']]]:
            ...
            if len(message.mentions) > 9 and not [item for item in message.content.strip().split() if item not in [msg_author.mention for msg_author in message.mentions]]:
                ...
                await self.safe_send_message(self.get_channel(CHANS['modmail']), content=MSGS['modmailaction'].format(action_log_id=CHANS['actions'], username=message.author.mention, id=message.author.id, reason='as they were muted'))
            ...
            try:
                if 'discord.gg' in item or 'discordapp.com/invite' in item:
                    ...
                    if invite.guild.id not in self.guild_whitelist:
                        if not [role for role in message.author.roles if role.id  in [ROLES['devs'], ROLES['help'], ROLES['bots']]]:
                            await self.safe_delete_message(message)
                            print('detected illegal invite from {}:{}\t{}'.format(message.author.name, message.author.id, item))
                            msg_to_send = 'I\'ve deleted your message in {} since I detected an invite url in your message! Please remember not to advertise servers not previously approved!'.format(message.channel.mention)
                            if message.author.id in self.mod_mail_db:
            ...
            if tag_name in self.tags:
                if self.tags[tag_name][0]:
                    ...
                    if "unrestricted_eval" in tag_flags:
                        ...
                        return
                    elif "restrict" in tag_flags and not [role for role in message.author.roles if role.id  in [ROLES['devs'], ROLES['help']]]:
                        return
                    elif "eval" in tag_flags:
                        resp = await self.cmd_eval(message.author, message.guild, message, message.channel, message.mentions, self.tags[tag_name][1], is_origin_tag=True)
                        if resp:
